...
const PowerLiftingPlans = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch('http://localhost:1337/api/power-lifting-plans?pagination[withCount]=true&populate=*');
        if (!response.ok) {
  ...
  return (
    <div>
      <h1 style={{color:'red'}}>Power Lifting Plans</h1>
...
export default PowerLiftingPlans;
